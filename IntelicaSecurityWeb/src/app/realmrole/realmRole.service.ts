...
export class RealmRoleService {
	...
	GetByFilter(realmID: string, realmRoleID: string, realmROleName: string): Observable<RealmRoleSimpleResponse[]> {
		...
		return this._http.get<RealmRoleSimpleResponse[]>(
			`${this._configService.environment?.apiPath}/RealmRole/GetByFilter/${realmID}/${realmRoleID}/${realmROleName}`
		);
	}
	...
	Find(realmRoleID: string): Observable<RealmRole> {
		return this._http.get<RealmRole>(`${this._configService.environment?.apiPath}/RealmRole/Find/${realmRoleID}`);
	}
	...
	Delete(realmRoleID: string): Observable<boolean> {
		return this._http.delete<boolean>(`${this._configService.environment?.apiPath}/RealmRole/${realmRoleID}`);
	}
	GetRealmRolesByRealm(realmID: string): Observable<RealmRoleSimpleResponse[]> {
		return this._http.get<RealmRoleSimpleResponse[]>(`${this._configService.environment?.apiPath}/RealmRole/GetRealmRolesByRealm/${realmID}`);
	}
}
