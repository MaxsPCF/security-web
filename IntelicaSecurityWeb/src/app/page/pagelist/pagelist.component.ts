...
export class PagelistComponent implements OnInit {
	...
	Export() {
		...
		let body: string = "<tr><th>Page name</th><th>Page url</th><th>Page root</th></tr>";
		this.Pages.forEach(row => {
			body += `<tr><td>${row.pageName}</td><td>${row.pageUrl}</td>td>${row.pageRoot}</td></tr>`;
		});
	...
	DeleteRow(row: PageSimpleResponse) {
		this.sweetAlertService.confirmBox("Do you want to delete this page?", "Yes", "No").then(response => {
